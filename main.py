...
# 避免 Milvus/gRPC C-core 日志插入交互式 input 提示行。
...
async def interactive_mode():
    ...
    while True:
        try:
            ...
            result = await coordinator.process(user_input, session_id=session_id)
            end_time = time.time()
            ...
            logger.exception(f"处理请求时出错: {e!r}")
            print(f"\n❌ 抱歉，处理您的问题时出现错误：{e!r}\n")
