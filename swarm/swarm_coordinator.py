...
class SwarmCoordinator:
    ...
    async def process(
        ...
        await self._emit_progress(
            ...
            "读取当前会话短期记忆",
            18,
            [{"name": "ShortTermMemory", "state": "读取会话上下文"}],
        )
        ...
        await self._emit_progress(
            ...
            "检索长期记忆中的相似案例",
            28,
            [{"name": "LongTermMemory", "state": "搜索相似案例"}],
        )
        ...
        await self._emit_progress(
            progress_callback,
            "dispatch",
            f"已生成 {len(subtasks)} 个子任务",
            48,
            [{"name": task.get("assigned_agent", "unknown_agent"), "state": "已分配"} for task in subtasks],
        )
        ...
        if len(subtasks) == 1:
            ...
            await self._emit_progress(
                ...
                f"路由到 {agent_id} 执行分析",
                62,
                [{"name": agent_id or "consultation_agent", "state": "执行中"}],
                "单 Agent",
            )
            ...
            await self._emit_progress(
                ...
                "检查回答结构和医疗免责声明",
                84,
                [{"name": agent_id or "consultation_agent", "state": "回答已生成"}],
                "单 Agent",
            )
        ...
        try:
            await self._emit_progress(
                ...
                "保存会话摘要和长期记忆",
                94,
                [{"name": "LongTermMemory", "state": "写入摘要"}],
                mode,
            )
    ...
    async def _process_with_swarm(
        ...
        await self._emit_progress(
            progress_callback,
            "analysis",
            f"{len(subtasks)} 个 Worker 任务开始并行执行",
            64,
            [{"name": task.assigned_agent, "state": "等待执行"} for task in subtasks],
            "协作",
        )
        ...
        # 即使超时，也尝试汇总已完成的部分结果
        completed_agents_before_synthesis = list(shared_context.agent_contributions.keys())
        await self._emit_progress(
            progress_callback,
            "safety",
            "整合 Agent 贡献并进行安全提示检查",
            84,
            [{"name": agent_id, "state": "贡献已完成"} for agent_id in completed_agents_before_synthesis],
            "协作",
        )
