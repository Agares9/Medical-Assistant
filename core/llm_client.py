        ...
        **kwargs
        ...
        try:
            ...
            if not content.strip():
                logger.warning(f"LLM returned empty content (finish_reason={choice.finish_reason})")
            logger.debug(f"LLM response length: {len(content)} chars")
            return content

        except Exception as e:
            logger.exception(f"LLM call failed: {e!r}")
            raise
    ...
    async def chat_with_tools(
        ...
        **kwargs
        ...
        try:
            ...
            logger.exception(f"LLM call with tools failed: {e!r}")
            raise
