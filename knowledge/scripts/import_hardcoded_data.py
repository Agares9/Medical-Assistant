    ...
    for txt_file in txt_files:
        try:
            ...
            if file_num.startswith('0') and int(file_num) < 10:
                ...
                if "icd11" in filename.lower() or "ICD-11" in content:
                    source = "WHO ICD-11 MMS API"
                elif "icd10" in filename.lower() or "ICD-10" in content:
...
def main():
    ...
    classification_docs = extract_documents_by_type(all_docs, "disease_classification")
    ...
    logger.info(f"   - 疾病分类编码: {len(classification_docs)}")
    ...
    test_queries = [
        ("血压高怎么办", "lifestyle"),
        ("糖尿病 ICD-11 编码", "disease_classification"),
